...
Breastcancer_model = joblib.load(open('Breastcnacer_rf_model.sav', 'rb'))
...
if (selected == 'Breast_cancer predciction'):
    ...
    with col1:
        #SkinThickness = st.text_input('Concave Points_Mean')
        SkinThickness = st.selectbox("Concave Points_Mean", ["0", "1"])
    with col2:
        Insulin = st.selectbox("Fractal_Dimension_Mean", ["0", "1"])
        #Insulin = st.text_input('fractal_dimension_mean')
    
    with col3:
        BMI= st.slider('Radius_See', 0, 2)
        #BMI = st.selectbox("Radius_See", ["0", "1","2"])
       # BMI = st.text_input('Radius_See')
